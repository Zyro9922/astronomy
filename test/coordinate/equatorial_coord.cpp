...
BOOST_AUTO_TEST_CASE(equatorial_coord_quantities_right_ascension_constructor)
{
    ...
    //Quantity stored as expected?
    BOOST_TEST((std::is_same<decltype(e1.get_X().get_angle()), quantity<si::plane_angle>>::value));
    ...
    //Quantity stored as expected?
    BOOST_TEST((std::is_same<decltype(e2.get_X().get_angle()), quantity<si::plane_angle>>::value));
